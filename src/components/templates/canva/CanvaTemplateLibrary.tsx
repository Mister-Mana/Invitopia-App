...
const CanvaTemplateLibrary: React.FC = () => {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      // Simulation de templates pour la démo
      const mockTemplates: Template[] = [
        {
          ...
          preview_images: ['/api/placeholder/300/400'],
        ...
        },
        {
          ...
          preview_images: ['/api/placeholder/400/600'],
  ...
  const useTemplate = (template: Template) => {
    ...
    navigate(`/templates/editor/new-${template.id}?name=${encodeURIComponent(template.name)}&type=${template.type}`);
  };
